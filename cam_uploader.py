...
GPIO.setup(BUTTON_PIN, GPIO.IN) # Uses internal resistor
...
def toggle_light(channel):
    ...
    if manual_override:
        ...
        print("Manual Override: LED ON")
    else:
        GPIO.output(LED_PIN, GPIO.LOW)
        print("Manual Override: OFF (Back to Flashlight mode)")

# Setup the "Listen" for button press (Falling edge because of ext. Pull UP)
...
def process_camera(cam):
    ...
    with open(out_file, "rb") as f:
        ...
        if r.status_code not in [200, 204]:
            print(f"Server rejected {cam['name']} with code {r.status_code}: {r.text}")
            r.raise_for_status()
        else:
            print(f"Successfully uploaded {cam['name']}")

if __name__ == "__main__":
    # Initialize all cameras
    for c in CAMERAS: 
        try: update_cam_info(c)
        except: print(f"Failed to init {c['name']}")
    ...
    while True:
        ...
        for c in CAMERAS:
            ...
            try:
                process_camera(c)
                print(f"Updated {c['name']} ({c['width']}x{c['height']})")
